...
module.exports = class Log {
  ...
  static async insert(log) {
    const { rows } = await pool.query('INSERT into logs (recipe_id, date_of_event, notes, rating) VALUES ($1, $2, $3, $4) RETURNING *', [log.recipeId, log.dateOfEvents, log.notes, log.rating]
    );
  ...
  static async findById(id) {
    ...
    if(!rows[0]) return null; else return new Log(rows[0]);
  }
